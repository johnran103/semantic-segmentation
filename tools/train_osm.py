...
from semseg.augmentations import get_train_augmentation_osm, get_val_augmentation_osm
from semseg.losses import get_loss
from semseg.schedulers import get_scheduler
from semseg.optimizers import get_optimizer
from semseg.utils.utils import fix_seeds, setup_cudnn, cleanup_ddp, setup_ddp
from val import evaluate, evaluate_osm
...
def main(cfg, gpu, save_dir):
    ...
    traintransform = get_train_augmentation_osm(train_cfg['IMAGE_SIZE'], seg_fill=dataset_cfg['IGNORE_LABEL'])
    valtransform = get_val_augmentation_osm(eval_cfg['IMAGE_SIZE'])
    ...
    model = eval(model_cfg['NAME'])(model_cfg['BACKBONE'], model_cfg['BACKBONE'], trainset.n_classes)
    ...
    valloader = DataLoader(valset, batch_size=1, num_workers=1, pin_memory=False)
    ...
    for epoch in range(epochs):
        ...
        if (epoch+1) % train_cfg['EVAL_INTERVAL'] == 0 or (epoch+1) == epochs:
            miou = evaluate_osm(model, valloader, device)[-1]
